...
class Robot(Problem):
    ...
    def result(self, state, action):
        ...
        act = action.split("_") #GET_A ["GET","A"]
        room = state["robot"]["room"]
        if act[0] == 'PUT':
            obj = act[1]
            newstate["rooms"][room].append(obj)
    ...
    def goal_test(self, state):
        """ Given a state, return True if state is a goal state or False, otherwise """
        if state["robot"]["room"] == self.goal["robot"]["room"] and\
            state["robot"]["hand"] == self.goal["robot"]["hand"]:
            return True
        return False

"""sample instances:
    {"robot":{"room":"A","hand":None},"rooms":{"A":["obj1"], "B":["obj2"], "C":["obj3","obj4"], "D":["obj5","obj6","obj7"], "E":["obj8","obj9"]}},{"robot":{"room":"A","hand":"obj1"}}
    {"robot":{"room":"A","hand":None},"rooms":{"A":["obj1"], "B":["obj2"], "C":["obj3","obj4"], "D":["obj5","obj6","obj7"], "E":["obj8","obj9"]}},{"robot":{"room":"B","hand":None}}
    {"robot":{"room":"A","hand":None},"rooms":{"A":["obj1"], "B":["obj2"], "C":["obj3","obj4"], "D":["obj5","obj6","obj7"], "E":["obj8","obj9"]}},{"robot":{"room":"B","hand":"obj1"}}
    {"robot":{"room":"B","hand":None},"rooms":{"A":["obj1"], "B":["obj2"], "C":["obj3","obj4"], "D":["obj5","obj6","obj7"], "E":["obj8","obj9"]}},{"robot":{"room":"D","hand":"obj2"}}
"""
#_______________________________________________________________________________
class Robot_complete(Problem):
    """State is defined as a dict:
    {
        "robot":{room, hand}
        "rooms":{room:[objects]}
...
class Robot_Map(Problem):
    ...
    def actions(self, state):
        ...
        for next_room in self.rooms[room]:
            possible_actions.append("MOVE_"+room+"_"+next_room)
            print("MOVE_"+next_room)
        #if roomIndex < len(self.rooms) -1:
            #possible_actions.append("MOVE_"+room+"_"+self.rooms[roomIndex+1])
        return possible_actions
...
#_______________________________________________________________________________
class Robot_MultipleObj(Problem):
    """State is defined as a dict:
    {
        "robot":{room, hand:[], current_load}
        "rooms":{room:[objects]}
    }
     """
    def __init__(self, initial, goal):
        ...
        Problem.__init__(self, initial, goal)
        self.rooms = ["A","B","C","D","E"]
        self.robot_max_load = 10

    def actions(self, state):
        possible_actions = []
        room = state["robot"]["room"]
        hand = state["robot"]["hand"]
        current_robot_load = state["robot"]["currentLoad"]
        if current_robot_load < self.robot_max_load:
            for obj in state["rooms"][room]:
                #obj obj1/10
                obj_params = obj.split("/")
                obj_name = obj_params[0]
                obj_weight = int(obj_params[1])
                if current_robot_load + obj_weight <= self.robot_max_load:  
                    #GET_obj1/10
                    possible_actions.append("GET_"+obj)
        #hand = ["obj1/10","obj3/5"]
        if current_robot_load > 0: #in python non è strettamente necessario fare questo ciclo
            for obj in hand:
                #"PUT_obj1/10" "PUT_obj3/5"
                possible_actions.append("PUT_"+obj)
        roomIndex = self.rooms.index(room)
        if roomIndex > 0:
            possible_actions.append("MOVE_"+room+"_"+self.rooms[roomIndex-1])
        if roomIndex < len(self.rooms) -1:
    ...
    def result(self, state, action):
        ...
        if act[0] == 'PUT':
            ...
            obj_weight = int(obj_params[1])
            newstate["rooms"][room].append(obj)
            newstate["robot"]["hand"].remove(obj)
            newstate["robot"]["currentLoad"] -= obj_weight
        elif act[0] == 'GET':
            obj = act[1]
            obj_params = obj.split("/")
            obj_name = obj_params[0]
            obj_weight = int(obj_params[1])
            newstate["rooms"][room].remove(obj)
            newstate["robot"]["hand"].append(obj)
            newstate["robot"]["currentLoad"] += obj_weight
        elif act[0] == "MOVE":
            current_room = act[1]
            next_room = act[2]
    ...
    def goal_test(self, state):
        """ Given a state, return True if state is a goal state or False, otherwise """
        if state["robot"]["room"] == self.goal["robot"]["room"] and\
            set(state["robot"]["hand"]) == set(self.goal["robot"]["hand"]) and\
            set(state["rooms"]["A"]) == set(self.goal["rooms"]["A"]) and\
            set(state["rooms"]["B"]) == set(self.goal["rooms"]["B"]) and\
            set(state["rooms"]["C"]) == set(self.goal["rooms"]["C"]) and\
            set(state["rooms"]["D"]) == set(self.goal["rooms"]["D"]) and\
            set(state["rooms"]["E"]) == set(self.goal["rooms"]["E"]): #controllo che anche le stanze siano come vogliamo
            return True
        return False
    #la funzione set prende una qualche collezione e la trasforma in un insieme in cui non conta l'ordine e
    #gli elementi ripetuti sono eliminati
#_______________________________________________________________________________
